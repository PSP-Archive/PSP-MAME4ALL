...
/*    RF5C68 start                              */
/************************************************/
int RF5C68_sh_start( struct RF5C68interface *inintf )
{
	int i;
	...
	if(pcmbuf==NULL) pcmbuf=(unsigned char *)gp2x_malloc(0x10000);
...

	{
		...
		name[0] = buf[0];
		name[1] = buf[1];
		sprintf( buf[0], "%s Left", "RF5C68" );
		sprintf( buf[1], "%s Right", "RF5C68" );
...
#define MIXER_PAN_RIGHT  2
		...
		stream = stream_init_multi( RF_LR_PAN, name, rate, 16, 0, RF5C68Update );
...
{
	if(pcmbuf!=NULL) gp2x_free(pcmbuf);
	pcmbuf=NULL;
}

void RF5C68_sh_update(void)
{
...
void RF5C68Update( int num, void **buffer, int length )
{
	...
	fast_memset( datap[RF_L_PAN], 0x00, length * sizeof(RF5C68_SAMPLE) );
	fast_memset( datap[RF_R_PAN], 0x00, length * sizeof(RF5C68_SAMPLE) );

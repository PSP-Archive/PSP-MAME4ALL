...
int bzone_sh_start(const struct MachineSound *msound)
{
	...
	discharge = (INT16 *)gp2x_malloc(32768 * sizeof(INT16));
	if( !discharge )
        return 1;
	...
	channel = stream_init("Custom", Machine->sample_rate, Machine->sample_bits, 0, (void (*)(int,void*,int))bzone_sound_update);
    if( channel == -1 )
        return 1;

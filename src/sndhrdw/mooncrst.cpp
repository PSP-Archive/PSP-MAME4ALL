...
void mooncrst_pitch_w(int offset,int data)
{
	...
	if (pitch) sample_adjust(channel+0,pitch,TOOTHSAW_VOLUME);
	else sample_adjust(channel+0,1000,0);
}
...
void mooncrst_vol_w(int offset,int data)
{
	...
	{
		vol = newvol;
		if (pitch) sample_setup(channel+0,waveform1[vol],TOOTHSAW_LENGTH,pitch,TOOTHSAW_VOLUME,1);
		else sample_setup(channel+0,waveform1[vol],TOOTHSAW_LENGTH,1000,0,1);
	}
}
...
void mooncrst_noise_w(int offset,int data)
{
        if (deathsampleloaded)
        {
           if (data & 1 && !(LastPort1 & 1))
              sample_setup(channel+1,Machine->samples->sample[1]->data,
        ...
        {
  	  if (data & 1) sample_adjust(channel+1,NOISE_RATE,NOISE_VOLUME);
	  else sample_adjust(channel+1,NOISE_RATE,0);
        }
}
...
void mooncrst_shoot_w(int offset,int data)
{
      ...
      if (data & 1 && !(LastPort2 & 1) && shootsampleloaded)
         sample_setup(channel+2,Machine->samples->sample[0]->data,
...
int mooncrst_sh_start(void)
{
	...
	if ((noise = (signed char*)gp2x_malloc(NOISE_LENGTH)) == 0)
	{
		return 1;
	}
	...
	sample_setup(channel+0,waveform1[vol],TOOTHSAW_LENGTH,1000,0,1);

	if (!deathsampleloaded)
		sample_setup(channel+1,noise,NOISE_LENGTH,NOISE_RATE,0,1);

	sample_setup(channel+3,waveform2,32,1000,0,1);
	sample_setup(channel+4,waveform2,32,1000,0,1);
...
{
	gp2x_free(noise);
	sample_stop(channel+0);
	sample_stop(channel+1);
	sample_stop(channel+2);
	sample_stop(channel+3);
	sample_stop(channel+4);
}
...
void mooncrst_sh_update(void)
{
    if (lfo_active)
    {
      sample_adjust(channel+3,freq*32,LFO_VOLUME);
      sample_adjust(channel+4,(freq+60)*32,LFO_VOLUME);
      if (t==0)
         freq-=lfo_rate;
      if (freq<=MINFREQ)
         freq=MAXFREQ;
    ...
    {
      sample_adjust(channel+3,1000,0);
      sample_adjust(channel+4,1000,0);
    }
    t++;
    if (t==3) t=0;

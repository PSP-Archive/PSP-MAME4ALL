...
void warpwarp_sound_w(int offs, int data)
{
...
void warpwarp_music1_w(int offs, int data)
{
    stream_update(channel,0);
...
void warpwarp_music2_w(int offs, int data)
{
    stream_update(channel,0);
	music2_latch = data;
	music_volume = 0x7fff;
...
static void warpwarp_sound_update(int param, void *buffer, int length)
{
	static int vcarry = 0;
	static int vcount = 0;
	static int mcarry = 0;
    ...
    while (length--)
    {
		*buf++ = (sound_signal + music_signal) / 2;
		/* *buffer++ = (unsigned char)(((sound_signal + music_signal) / 2) >> 8) ^ 0x80; */
...
int warpwarp_sh_start(void)
{
	int i;

	decay = (INT16 *) gp2x_malloc(0x8000 * sizeof(INT16));
	if( !decay )
		return 1;
	...
	channel = stream_init("WarpWarp", Machine->sample_rate, Machine->sample_bits, 0, warpwarp_sound_update);

...
int llander_sh_start(void)
{
	...
	lfsr_index=0;
	if ((lfsr_buffer = (short unsigned *)gp2x_malloc(65536*2)) == 0) return 1;
	...
	if ((sample_buffer = gp2x_malloc((Machine->sample_bits/8)*buffer_len)) == 0) return 1;
	fast_memset(sample_buffer,0,(Machine->sample_bits/8)*buffer_len);
...
void llander_sh_update(void)
{
	...
	{
	        osd_play_streamed_sample(channel,(signed char *)sample_buffer,buffer_len,emulation_rate,0xff,OSD_PAN_CENTER);
	}
}

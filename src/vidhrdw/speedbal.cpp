...
{
	if ((bg_dirtybuffer = (unsigned char *)gp2x_malloc (speedbal_background_videoram_size)) == 0)
	{
		return 1;
	}
	if ((ch_dirtybuffer = (unsigned char *)gp2x_malloc (speedbal_foreground_videoram_size)) == 0)
	{
	...
	fast_memset (ch_dirtybuffer,1,speedbal_foreground_videoram_size / 2);
	fast_memset (bg_dirtybuffer,1,speedbal_background_videoram_size / 2);
	return 0;

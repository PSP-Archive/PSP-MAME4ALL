...
int foodf_vh_start(void)
{
	...
	if (!playfielddirty) playfielddirty = (unsigned char *)gp2x_malloc (foodf_playfieldram_size / 2);
	if (!playfielddirty)
	{
		foodf_vh_stop ();
		return 1;
	}
	fast_memset (playfielddirty, 1, foodf_playfieldram_size / 2);
...
void foodf_vh_stop(void)
{
	...
	/* free dirty buffers */
	if (playfielddirty) gp2x_free (playfielddirty); playfielddirty = 0;
}

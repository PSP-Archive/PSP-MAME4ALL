...
int taito_vh_start(void)
{
	...
	if ((dirtybuffer2 = (unsigned char *)gp2x_malloc(videoram_size)) == 0)
	{
		generic_vh_stop();
		return 1;
	}
	fast_memset(dirtybuffer2,1,videoram_size);

	if ((dirtybuffer3 = (unsigned char *)gp2x_malloc(videoram_size)) == 0)
	{
		gp2x_free(dirtybuffer2);
		generic_vh_stop();
		return 1;
	}
	fast_memset(dirtybuffer3,1,videoram_size);

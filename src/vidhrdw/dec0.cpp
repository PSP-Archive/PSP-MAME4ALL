...
static void dec0_update_palette(int pf23priority)
{
	...
	if (palette_recalc())
	{
		fast_memset(dec0_pf1_dirty,1,TEXTRAM_SIZE);
		fast_memset(dec0_pf2_dirty,1,TILERAM_SIZE);
		fast_memset(dec0_pf3_dirty,1,TILERAM_SIZE);
	}
}
...
void dec0_vh_stop (void)
{
	...
	osd_free_bitmap(dec0_tf2_bitmap);
	osd_free_bitmap(dec0_tf3_bitmap);
	gp2x_free(dec0_pf3_dirty);
	gp2x_free(dec0_pf2_dirty);
	gp2x_free(dec0_pf1_dirty);
}
...
int dec0_vh_start (void)
{
	...
	dec0_pf1_dirty = (unsigned char *)gp2x_malloc(TEXTRAM_SIZE);
	dec0_pf3_dirty = (unsigned char *)gp2x_malloc(TILERAM_SIZE);
	dec0_pf2_dirty = (unsigned char *)gp2x_malloc(TILERAM_SIZE);

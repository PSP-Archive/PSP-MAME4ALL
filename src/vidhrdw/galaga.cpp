...
struct star
{
	int x,y,col,set,lx,ly;
};
static struct star stars[MAX_STARS];
static int total_stars;

#define MAX_SPRITES 64
static struct rectangle dirtyrects[MAX_SPRITES];
		...
		for (x = 511;x >= 0;x--)
		{
			...
			if (((~generator >> 16) & 1) && (generator & 0xff) == 0xff)
			{
				...
				if (color && total_stars < MAX_STARS)
				{
					...
					stars[total_stars].lx = -1;
					stars[total_stars].ly = -1;
					if (++set > 3)
						set = 0;
...
void galaga_vh_screenrefresh(struct osd_bitmap *bitmap,int full_refresh)
{
	...
	for (offs = videoram_size - 1;offs >= 0;offs--)
	{
		...
		/* checking is performed by drawgfx(). */
			...
			/* If we're updating less than 32 chars, copy chars one at a time */
			if (numchars < 32)
			{
				...
				copybitmap(bitmap,tmpbitmap,0,0,0,0,&rect,TRANSPARENCY_NONE,0);
			}
		}	
	}

	/* copy the whole char bitmap if we're doing more than 32 */
	if (numchars >= 32)
		copybitmap(bitmap,tmpbitmap,0,0,0,0,&Machine->drv->visible_area,TRANSPARENCY_NONE,0);
	else if (galaga_starcontrol[5] & 1)
	{
		/* We're updating only some of the character layer.. do dirty rects */
		...
		for (offs = 0;offs < total_stars;offs++)
		{
			...
			{
				y = stars[offs].ly;
				bitmap->line[y][x] = tmpbitmap->line[y][x];
			}
		}

		/* Clear sprite dirty rects. */
		for (offs = 0;offs < spriteram_size;offs += 2)
		{
			dirty = &dirtyrects[offs>>1];
			if (dirty->min_x < dirty->max_x)
				copybitmap(bitmap,tmpbitmap,0,0,0,0,dirty,TRANSPARENCY_NONE,0); 
			dirty->min_x = dirty->max_x = 0;
		}
	...
	for (offs = 0;offs < spriteram_size;offs += 2)
	{
		...
		if ((spriteram_3[offs + 1] & 2) == 0)
		{
			...
			if ((spriteram_3[offs] & 0x0c) == 0x0c)		/* double width, double height */
			{
				...
				dirty->min_x = sx;
				/*dirty->min_y = min(sy-sfa,sy-sfb);*/
				dirty->min_y = sy-sfa < sy-sfb ? sy-sfa : sy-sfb;
				dirty->max_x = sx+31;
				dirty->max_y = sy+31;
			}
	...
	if (galaga_starcontrol[5] & 1)
	{
		...
		for (offs = 0;offs < total_stars;offs++)
		{
			...
			{
				x = (((stars[offs].x + stars_scroll) & 511) >> 1) + 16;
				y = (stars[offs].y + ((stars_scroll + stars[offs].x) >> 9)) & 255;

...
{
        if ((fg_dirtybuffer = (unsigned char *)gp2x_malloc(32*32)) == 0)
	{
        	return 1;
	}
        if ((bg_dirtybuffer = (unsigned char *)gp2x_malloc(32*32)) == 0)
	{
	...
	if ((bitmap_bg = osd_new_bitmap (Machine->drv->screen_width,Machine->drv->screen_height,Machine->scrbitmap->depth)) == 0)
	{
        ...
        fast_memset(fg_dirtybuffer,1,32*32);
        fast_memset(bg_dirtybuffer,1,32*32);
        return 0;
}

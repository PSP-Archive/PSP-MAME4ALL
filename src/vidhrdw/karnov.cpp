...
{
	if (dirty_f) gp2x_free(dirty_f);
	if (karnov_foreground) gp2x_free(karnov_foreground);
...
int karnov_vh_start (void)
{
	...
	dirty_f=(unsigned char *)gp2x_malloc(0x1000);
	karnov_foreground=(unsigned char *)gp2x_malloc(0x1000);
	fast_memset(karnov_foreground,0,0x1000);
	fast_memset(dirty_f,1,0x1000);

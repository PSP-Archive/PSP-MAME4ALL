...
static long mask_buffer_alloc( long size ){
	...
	if( req_size>mask_buffer_size ){
		...
		unsigned char *temp=(unsigned char *)gp2x_malloc(req_size);
		fast_memcpy(temp,mask_buffer,sizeof(mask_buffer));
		gp2x_free(mask_buffer);
		mask_buffer=temp;
...
static void do_blit_zoom( const struct sprite *sprite ){
	...
	//printf("x1=%d,x2=%d,dx=%d,xcount0=%d,y1=%d,y2=%d,dy=%d,ycount0=%d,flags=%d\n",x1,x2,dx,xcount0,y1,y2,dy,ycount0,sprite->flags);fflush(stdout);
...
void sprite_close( void ){
	...
	while( sprite_list ){
		...
		gp2x_free( sprite_list->sprite );
		gp2x_free( sprite_list );
		sprite_list = next;
	}
...
struct sprite_list *sprite_list_create( int num_sprites, int flags ){
	...
	struct sprite *sprite = (struct sprite *) gp2x_malloc (num_sprites*sizeof(struct sprite));
	fast_memset(sprite,0,num_sprites*sizeof(struct sprite));
	//struct sprite_list *sprite_list = (struct sprite_list *) calloc( 1, sizeof(struct sprite_list) );
	struct sprite_list *sprite_list = (struct sprite_list *) gp2x_malloc( 1*sizeof(struct sprite_list) );
	fast_memset(sprite_list,0,1*sizeof(struct sprite_list));

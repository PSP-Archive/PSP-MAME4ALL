...
{
	game_palette = (unsigned char*)gp2x_malloc(3 * Machine->drv->total_colors * sizeof(unsigned char));
	palette_map = (unsigned short*)gp2x_malloc(Machine->drv->total_colors * sizeof(unsigned short));
	if (Machine->drv->color_table_len)
	{
		game_colortable = (unsigned short*)gp2x_malloc(Machine->drv->color_table_len * sizeof(unsigned short));
		Machine->colortable = (unsigned short*)gp2x_malloc(Machine->drv->color_table_len * sizeof(unsigned short));
	}
	else game_colortable = Machine->colortable = 0;

	/* ASG 980209 - allocate space for the pens */
	shrinked_pens = (unsigned short*)gp2x_malloc(STATIC_MAX_PENS * sizeof (short));

	Machine->pens = (unsigned short*)gp2x_malloc(Machine->drv->total_colors * sizeof (short));
			...
			Machine->drv->total_colors > DYNAMIC_MAX_PENS)
	{
		...
		palette_used_colors = (unsigned char*)gp2x_malloc(3 * Machine->drv->total_colors * sizeof(unsigned char));

		pen_visiblecount = (int *)gp2x_malloc(2 * Machine->drv->total_colors * sizeof(int));
		...
		fast_memset(old_used_colors,PALETTE_COLOR_UNUSED,Machine->drv->total_colors * sizeof(unsigned char));

		pen_cachedcount = pen_visiblecount + Machine->drv->total_colors;
		fast_memset(pen_visiblecount,0,Machine->drv->total_colors * sizeof(int));
		fast_memset(pen_cachedcount,0,Machine->drv->total_colors * sizeof(int));
	}
...
{
	if (palette_used_colors) gp2x_free(palette_used_colors);
	palette_used_colors = old_used_colors = just_remapped = 0;
	gp2x_free(pen_visiblecount);
	pen_visiblecount = 0;
	gp2x_free(game_palette);
	game_palette = 0;
	gp2x_free(palette_map);
	palette_map = 0;
	gp2x_free(game_colortable);
	game_colortable = 0;
	gp2x_free(Machine->colortable);
	Machine->colortable = 0;
	gp2x_free(shrinked_pens);
	shrinked_pens = 0;
	gp2x_free(Machine->pens);
	Machine->pens = 0;
}
...
void palette_init(void)
{
	...
	{
		fast_memcpy(game_palette,Machine->gamedrv->palette,3 * Machine->drv->total_colors * sizeof(unsigned char));
		if (Machine->gamedrv->colortable)
			fast_memcpy(game_colortable,Machine->gamedrv->colortable,Machine->drv->color_table_len * sizeof(unsigned short));
	}
	else
	{
...
static void build_rgb_to_pen(void)
{
	...
	fast_memset(rgb6_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb6_to_pen));
	rgb6_to_pen[0][0][0] = BLACK_PEN;
	fast_memset(rgb2_to_pen,BLACK_PEN,sizeof(rgb2_to_pen));
	fast_memset(rgb3_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb3_to_pen));
	fast_memset(rgb4_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb4_to_pen));
	fast_memset(rgb5_to_pen,DYNAMIC_MAX_PENS,sizeof(rgb5_to_pen));
...
const unsigned char *palette_recalc(void)
{
	...
	fast_memset(just_remapped,0,Machine->drv->total_colors * sizeof(unsigned char));

...
void op_50c8(void) /* DBcc */
{
	...
	if (!cctrue(0))
		...
		if (src--)
			{
			m68k_regs.pc += (LONG)offs - 2;
			}
		m68k_regs.d[srcreg].W.l = src;
		}
}
...
{	WORD offs = nextiword();
	if (!cctrue(6)) {
	if (src--) { m68k_regs.pc += (LONG)offs - 2; /*change_pc24(m68k_regs.pc&0xffffff);*/ }	/* ASG 971108 */
	m68k_regs.d[srcreg].W.l = src;
	}
}}}}
